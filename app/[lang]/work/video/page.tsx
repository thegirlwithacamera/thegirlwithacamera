...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: lang === "fr" ? "Vidéo · Sandrine Ceuppens" : "Video · Sandrine Ceuppens",
  };
}

export default async function WorkVideoPage({ params }: Props) {
  const { lang } = await params;
  const isFr = lang === "fr";
    ...
    <main style={{ paddingTop: "52px", minHeight: "70vh" }}>
      ...
      <div style={{ borderBottom: "1px solid #ebebeb", padding: "0 48px" }}>
        <div style={{ display: "flex", gap: "32px", maxWidth: "1280px", margin: "0 auto" }}>
          {[
            ...
            const active = tab.href.includes("video");
            return (
      ...
      <section style={{ maxWidth: "1280px", margin: "0 auto", padding: "80px 48px" }}>
        ...
        <p style={{ fontSize: "14px", color: "#9a9a9a", lineHeight: 1.8, maxWidth: "480px" }}>
          {isFr
            ? "Scripting, tournage, montage. Contenu de marque et éditorial vidéo. Bientôt disponible."
            : "Scripting, filming, editing. Brand and editorial video content. Coming soon."}
        </p>
      </section>
    </main>
  );
